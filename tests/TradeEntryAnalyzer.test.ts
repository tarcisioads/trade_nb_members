...
describe('TradeEntryAnalyzer', () => {
  ...
  beforeEach(() => {
    ...
    const MockDataServiceManager = DataServiceManager as jest.MockedClass<
      typeof DataServiceManager
    >;
    // The constructor is called inside TradeEntryAnalyzer, so we need to access the instance that was created
    mockDataServiceManager = MockDataServiceManager.mock
      .instances[0] as jest.Mocked<DataServiceManager>;
  ...
  describe('analyzeEntry', () => {
    it('should return correct analysis for LONG entry', async () => {
      ...
      mockDataServiceManager.getKlineData.mockResolvedValue({
        data: mockKlineData,
        source: 'binance',
      });
