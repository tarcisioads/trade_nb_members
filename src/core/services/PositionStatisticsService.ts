...
export class PositionStatisticsService {
  public calculateStats(positions: PositionHistory[]): any {
    ...
    positions.forEach((position) => {
      ...
      if (netProfit > 0) {
        ...
        grossProfitBySymbol[position.symbol] =
          (grossProfitBySymbol[position.symbol] || 0) + netProfit;
      } else if (netProfit < 0) {
        grossLossBySymbol[position.symbol] =
          (grossLossBySymbol[position.symbol] || 0) + Math.abs(netProfit);
      }
      ...
      if (netProfit > bestProfit) {
        ...
        bestTradeId =
          position.tradeInfo && position.tradeInfo.trade && position.tradeInfo?.found
            ? position.tradeInfo.trade.id
            : null;
      }
    ...
    const mostProfitableSymbol = Object.keys(symbolProfits).reduce(
      (a, b) => (symbolProfits[a] > symbolProfits[b] ? a : b),
      Object.keys(symbolProfits)[0] || ''
    );
    const mostProfitableSide = Object.keys(sideProfits).reduce(
      (a, b) => (sideProfits[a] > sideProfits[b] ? a : b),
      Object.keys(sideProfits)[0] || ''
    );
    ...
    if (sortedProfitableSymbols.length > 10) {
      const othersAmount = sortedProfitableSymbols
        .slice(10)
    ...
    if (sortedLosingSymbols.length > 10) {
      const othersAmount = sortedLosingSymbols
        .slice(10)
        .reduce((sum, item) => sum + item.amount, 0);
    ...
    return {
      ...
      riskMetrics: {
        totalRisk:
          risks.length > 0 ? Math.round(risks.reduce((sum, risk) => sum + risk, 0) * 100) / 100 : 0,
        avgRiskPerTrade:
          risks.length > 0
            ? Math.round((risks.reduce((sum, risk) => sum + risk, 0) / risks.length) * 100) / 100
            : 0,
        maxRiskPerTrade: risks.length > 0 ? Math.round(Math.max(...risks) * 100) / 100 : 0,
        riskRewardRatio:
          risks.length > 0 && riskRewardRatios.length > 0
            ? Math.round(
                (riskRewardRatios.reduce((sum, ratio) => sum + ratio, 0) /
        ...
        avgDrawdown:
          drawdowns.length > 0
            ? Math.round((drawdowns.reduce((sum, dd) => sum + dd, 0) / drawdowns.length) * 100) /
              100
            : 0,
      ...
      tradeMetrics: {
        ...
        avgEntryPrice:
          totalTradesWithInfo > 0
            ? Math.round((totalEntryPrice / totalTradesWithInfo) * 100) / 100
            : 0,
        avgStopPrice:
          totalTradesWithInfo > 0
            ? Math.round((totalStopPrice / totalTradesWithInfo) * 100) / 100
            : 0,
        avgTakeProfit1:
          totalTradesWithInfo > 0
            ? Math.round((totalTakeProfit1 / totalTradesWithInfo) * 100) / 100
            : 0,
        avgLeverage:
          totalTradesWithInfo > 0
            ? Math.round((totalLeverage / totalTradesWithInfo) * 100) / 100
            : 0,
        avgQuantity:
          totalTradesWithInfo > 0
            ? Math.round((totalQuantity / totalTradesWithInfo) * 100) / 100
            : 0,
  ...
  public calculateDetailedRiskStats(positions: any[]): any {
    ...
    const tradesWithInfo = positions.filter(
      (position) => position.tradeInfo && position.tradeInfo?.trade && position.tradeInfo?.found
    );
    ...
    Object.keys(symbolAnalysis).forEach((symbol) => {
      ...
      analysis.avgRisk =
        analysis.risks.length > 0
          ? analysis.risks.reduce((a: number, b: number) => a + b, 0) / analysis.risks.length
          : 0;
      analysis.avgReward =
        analysis.rewards.length > 0
          ? analysis.rewards.reduce((a: number, b: number) => a + b, 0) / analysis.rewards.length
          : 0;
      analysis.avgRR =
        analysis.rrRatios.length > 0
          ? analysis.rrRatios.reduce((a: number, b: number) => a + b, 0) / analysis.rrRatios.length
          : 0;
      analysis.avgLeverage =
        leverages
          .filter((_, i) => positions[i].symbol === symbol)
          .reduce((a: number, b: number) => a + b, 0) / analysis.totalTrades;
    });
    ...
    Object.keys(sideAnalysis).forEach((side) => {
      ...
      analysis.avgLeverage =
        leverages
          .filter((_, i) => positions[i].positionSide === side)
          .reduce((a: number, b: number) => a + b, 0) / analysis.totalTrades;
    });
    ...
    return {
      ...
      riskRewardAnalysis: {
        avgRiskRewardRatio:
          riskRewardRatios.length > 0
            ? Math.round(
                (riskRewardRatios.reduce((a, b) => a + b, 0) / riskRewardRatios.length) * 100
              ) / 100
            : 0,
        ...
        bestRiskRewardRatio:
          riskRewardRatios.length > 0 ? Math.round(Math.max(...riskRewardRatios) * 100) / 100 : 0,
        worstRiskRewardRatio:
          riskRewardRatios.length > 0 ? Math.round(Math.min(...riskRewardRatios) * 100) / 100 : 0,
      },
      performanceMetrics: {
        sharpeRatio: Math.round(sharpeRatio * 100) / 100,
      ...
      tradeAnalysis: {
        avgLeverage:
          leverages.length > 0
            ? Math.round((leverages.reduce((a, b) => a + b, 0) / leverages.length) * 100) / 100
            : 0,
