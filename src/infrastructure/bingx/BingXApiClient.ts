    ...
    try {
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        if (BingXApiClient.pauseRequestsUntil > now) {
          ...
          logger.warn(`Requests are globally paused due to rate limit. Waiting ${waitTime}ms...`, {
            path,
          });
        ...
        try {
          ...
          if (
            ...
            if (response.data.code === 80012) {
              ...
              if (unblockMatch) {
                ...
                if (waitTime > 0) {
                  // Update global pause
                  BingXApiClient.pauseRequestsUntil = Math.max(
                    BingXApiClient.pauseRequestsUntil,
                    unblockTimestamp + 1000
                  );
                  ...
                  logger.warn(
                    `Frequency limit hit (100410). Globally pausing until ${new Date(BingXApiClient.pauseRequestsUntil).toISOString()}`,
                    {
                      requestId,
                      path,
                      attempt,
                  ...
                  TelegramService.getInstance()
                    ...
                    .catch((err) =>
                      logger.error('Error sending rate limit notification to Telegram', err)
                    );
                ...
                logger.warn(
                  'Frequency limit hit (100410) with no unblock info. Globally pausing for 60s.',
                  { path }
                );
          ...
          if (axios.isAxiosError(error) && error.response?.data?.code === 100410) {
            ...
            const unblockMatch = /unblocked after (\d{13,})/.exec(msg);
            const unblockTimestamp = unblockMatch
              ? parseInt(unblockMatch[1], 10)
              : Date.now() + 60000;
            ...
            logger.warn(
              `Frequency limit hit (100410) in error. Globally pausing until ${new Date(BingXApiClient.pauseRequestsUntil).toISOString()}`,
              {
                path,
                unblockTimestamp,
              }
