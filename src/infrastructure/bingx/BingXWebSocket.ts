...
export class BingXWebSocket {
    ...
    this.pingInterval = setInterval(() => {
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        ...
        if (timeSinceLastPong > this.PING_INTERVAL * 2) {
          console.log(
            `No pong received for ${timeSinceLastPong}ms for ${this.symbol}, reconnecting...`
          );
          this.handleConnectionLoss();
        }
      }
  ...
  private handleConnectionLoss(): void {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(
        `Connection lost for ${this.symbol}. Attempting to reconnect in ${reconnectDelay / 1000}s... (Attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`
      );

      const now = Date.now();
      if (
        this.reconnectAttempts % 5 === 0 &&
        now - this.lastInstabilityAlertTime > this.ALERT_COOLDOWN_MS
      ) {
