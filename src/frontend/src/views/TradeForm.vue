<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div
      v-if="warningMessage"
      class="bg-yellow-500/10 border border-yellow-500/50 text-yellow-200 px-4 py-3 rounded mb-6 flex justify-between items-center animate-fade-in"
      role="alert"
    >
    ...
    <div class="max-w-2xl mx-auto glass-card animate-slide-up">
      ...
      <div class="p-6">
        <form class="space-y-6" @submit.prevent="saveTrade">
          <!-- Pair and Type -->
            ...
            <div class="flex flex-wrap items-end gap-4">
              <div class="flex-grow">
                <label class="block text-sm font-medium text-gray-400 mb-2"
                  >Suggest TP Interval</label
                >
                <select
                  v-model="suggestInterval"
                  class="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all outline-none appearance-none"
                >
                  <option value="5m">5m</option>
                  <option value="15m">15m</option>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
              <div v-for="i in 6" :key="i" class="relative">
                ...
                >
                <input
                  v-model.number="tradeData[`tp${i}` as keyof Trade]"
                  type="number"
                  step="any"
          ...
          <!-- Checkboxes -->
          <div
            class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-800/30 rounded-xl border border-white/5"
          >
            <div class="flex items-center gap-3">
              <input
              ...
              />
              <label class="text-gray-300 cursor-pointer select-none" for="volumeRequiredCheck"
                >Volume Required</label
              >
            </div>
            <div class="flex items-center gap-3">
              <input
                ...
                class="w-5 h-5 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500/50 focus:ring-offset-0"
              />
              <label class="text-gray-300 cursor-pointer select-none" for="volumeMarginCheck"
                >Volume Adds Margin</label
              >
            </div>
            <div class="flex items-center gap-3">
              <input
                id="sentimentRequiredCheck"
                v-model="tradeData.sentiment_required"
                type="checkbox"
                class="w-5 h-5 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500/50 focus:ring-offset-0"
              />
              <label class="text-gray-300 cursor-pointer select-none" for="sentimentRequiredCheck"
                >Sentiment Required</label
              >
            </div>
            <div class="flex items-center gap-3">
              <input
                id="sentimentMarginCheck"
                v-model="tradeData.sentiment_adds_margin"
                type="checkbox"
                class="w-5 h-5 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500/50 focus:ring-offset-0"
              />
              <label class="text-gray-300 cursor-pointer select-none" for="sentimentMarginCheck"
                >Sentiment Adds Margin</label
              >
            </div>
          </div>
          ...
          <div class="flex justify-end gap-4 pt-4 border-t border-white/10">
            ...
            <button type="submit" class="btn-primary px-8">Save Trade</button>
          </div>
        </form>
      </div>
    </div>
...
const handleSymbolBlur = () => {
  ...
  const existingTrade = allTrades.value.find(
    (t) => t.symbol.toLowerCase() === newSymbol.toLowerCase()
  );
