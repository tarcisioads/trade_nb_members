      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="text-center p-3 rounded-lg bg-white/5">
          <div class="text-2xl font-bold text-yellow-400">
            {{ formatNumber(stats.avgRiskPerTrade) }}
          </div>
          <div class="text-xs text-gray-400 mt-1">Avg Risk/Trade</div>
        </div>
        <div class="text-center p-3 rounded-lg bg-white/5">
          <div class="text-2xl font-bold text-blue-400">
            {{ formatNumber(stats.avgRiskRewardRatio) }}
          </div>
      ...
      <div class="mt-4 space-y-3">
        ...
        <div class="flex justify-between items-center bg-white/5 p-2 rounded">
          ...
          <span class="px-2 py-1 text-xs font-bold text-blue-900 bg-blue-400 rounded">{{
            formatNumber(stats.avgRiskReturnedPositive)
          }}</span>
        </div>
        <div class="flex justify-between items-center bg-white/5 p-2 rounded">
        ...
        <div class="flex justify-between items-center bg-white/5 p-2 rounded">
          ...
          <span class="px-2 py-1 text-xs font-bold text-gray-900 bg-gray-400 rounded">{{
            formatNumber(stats.bestRiskRewardRatio)
          }}</span>
        </div>
        <div class="flex justify-between items-center bg-white/5 p-2 rounded">
          ...
          <span class="px-2 py-1 text-xs font-bold text-yellow-900 bg-yellow-400 rounded">{{
            formatNumber(stats.worstRiskRewardRatio)
          }}</span>
        </div>
      </div>
    </div>
