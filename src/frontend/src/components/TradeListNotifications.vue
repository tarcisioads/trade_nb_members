          ...
          <div class="w-full">
            ...
            <div class="flex flex-wrap gap-3 text-sm">
              ...
              <span class="text-gray-300"
                >Entry: <span class="font-mono">{{ trade.entry }}</span></span
              >
              <span class="text-gray-300"
                >Stop: <span class="font-mono">{{ trade.stop }}</span></span
              >
              <span class="px-2 py-1 bg-gray-700 rounded text-gray-300 text-xs"
                >Interval: {{ trade.interval }}</span
              >
            </div>
            <div class="mt-3">
              ...
              <div class="flex flex-wrap gap-2 mt-1">
                <span
                  v-if="trade.takeProfits.tp1"
                  class="px-2 py-0.5 bg-blue-500/10 text-blue-300 rounded text-xs"
                  >TP1: {{ trade.takeProfits.tp1 }}</span
                >
                <span
                  v-if="trade.takeProfits.tp2"
                  class="px-2 py-0.5 bg-blue-500/10 text-blue-300 rounded text-xs"
                  >TP2: {{ trade.takeProfits.tp2 }}</span
                >
                <span
                  v-if="trade.takeProfits.tp3"
                  class="px-2 py-0.5 bg-blue-500/10 text-blue-300 rounded text-xs"
                  >TP3: {{ trade.takeProfits.tp3 }}</span
                >
                <span
                  v-if="trade.takeProfits.tp4"
                  class="px-2 py-0.5 bg-blue-500/10 text-blue-300 rounded text-xs"
                  >TP4: {{ trade.takeProfits.tp4 }}</span
                >
                <span
                  v-if="trade.takeProfits.tp5"
                  class="px-2 py-0.5 bg-blue-500/10 text-blue-300 rounded text-xs"
                  >TP5: {{ trade.takeProfits.tp5 }}</span
                >
                <span
                  v-if="trade.takeProfits.tp6"
                  class="px-2 py-0.5 bg-blue-500/10 text-blue-300 rounded text-xs"
                  >TP6: {{ trade.takeProfits.tp6 }}</span
                >
              </div>
            </div>
            <div class="mt-3">
              <div class="flex flex-wrap gap-2 items-center text-xs">
                ...
                <span
                  v-if="trade.isWarning"
                  class="px-2 py-1 bg-yellow-500/20 text-yellow-500 rounded font-bold"
                >
                  ⚠️ Warning
                </span>
                <span
                  :class="
                    trade.volume_required
                      ? 'bg-red-500/20 text-red-400'
                      : 'bg-green-500/20 text-green-400'
                  "
                ...
                <small class="text-gray-400 block w-full mt-1">{{
              ...
              <div class="mt-1 text-xs text-gray-400">
                Volume: {{ trade.validation.volumeAnalysis.currentVolume.toFixed(2) }} (std:
                {{ trade.validation.volumeAnalysis.stdBar.toFixed(2) }})
                <span
              ...
              <div v-if="trade.setup_description" class="mt-3">
                <div class="bg-white/5 rounded p-3 border border-white/10">
                  <h6 class="text-xs font-semibold text-gray-400 mb-1 uppercase tracking-top">
                    Setup Description
                  </h6>
            ...
            <div v-if="trade.executionResult" class="mt-3">
              <div class="bg-blue-500/10 rounded p-3 border border-blue-500/20">
                <h6 class="text-xs font-semibold text-blue-400 mb-2 uppercase tracking-wide">
                  Execution Results
                </h6>
                <div class="flex flex-col gap-1 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="font-bold text-white font-mono"
                      >{{ trade.executionResult.leverage }}x</span
                    >
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-400">Quantity:</span>
                    <span class="font-bold text-white font-mono">{{
                      trade.executionResult.quantity.toFixed(4)
                    }}</span>
                  </div>
                  <div
                    v-if="trade.volume_adds_margin && trade.executionResult.volumeMarginAdded"
                    class="mt-1 pt-1 border-t border-white/10"
                  >
                    <div class="flex justify-between">
                      <span class="text-gray-400">Volume Margin:</span>
                      <span class="font-bold text-green-400 font-mono"
                        >+{{ trade.executionResult.volumeMarginAdded.percentage }}%</span
                      >
                    </div>
                  </div>
                  <div
                    v-if="trade.sentiment_adds_margin && trade.executionResult.sentimentMarginAdded"
                    class="mt-1 pt-1 border-t border-white/10"
                  >
                    <div class="flex justify-between">
                      <span class="text-gray-400">Sentiment Margin:</span>
                      <span class="font-bold text-green-400 font-mono"
                        >+{{ trade.executionResult.sentimentMarginAdded.percentage }}%</span
                      >
                    </div>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-400">Entry Order:</span>
                    <span class="font-mono text-gray-300 truncate max-w-[150px]">{{
                      trade.executionResult.entryOrderId
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-400">Stop Order:</span>
                    <span class="font-mono text-gray-300 truncate max-w-[150px]">{{
                      trade.executionResult.stopOrderId
                    }}</span>
                  </div>
                </div>
              </div>
            ...
            <div v-if="trade.executionError" class="mt-3">
              <div
                class="bg-red-500/10 rounded p-3 border border-red-500/20 text-red-300 text-sm flex items-start gap-2"
              >
            ...
            <div class="mt-4 pt-3 border-t border-white/5">
              <div class="flex gap-2">
                <a
                  :href="trade.analysisUrl"
                  target="_blank"
                  class="px-3 py-1.5 border border-blue-500 text-blue-400 rounded hover:bg-blue-500/10 transition-colors text-sm"
                >
                  View Analysis
                </a>
                <template v-if="trade.isWarning">
                  <button
                    class="px-3 py-1.5 bg-green-600 text-white rounded hover:bg-green-700 transition-colors text-sm disabled:opacity-50"
                  ...
                  <button
                    class="px-3 py-1.5 bg-yellow-600 text-white rounded hover:bg-yellow-700 transition-colors text-sm disabled:opacity-50"
