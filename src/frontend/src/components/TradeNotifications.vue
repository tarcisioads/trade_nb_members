    ...
    <div class="glass-card">
      <div class="p-4 border-b border-white/10 flex justify-between items-center bg-blue-600/10">
        <div class="flex items-center gap-3">
          ...
          <router-link
            to="/notifications"
            class="px-3 py-1.5 border border-blue-500 text-blue-400 rounded hover:bg-blue-500/10 transition-colors text-sm flex items-center gap-2"
          >
            <i class="bi bi-clock-history"></i> History
          </router-link>
        </div>
        <span class="px-3 py-1 rounded text-sm font-bold" :class="connectionStatusClass">{{
          connectionStatus
        }}</span>
      </div>
      <div v-if="connectionStatus === 'Connected'" class="p-4">
        <TradeListNotifications :key="trades.length" :trades="trades" />
      </div>
    </div>
  </div>
</template>
