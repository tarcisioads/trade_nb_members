    ...
    <div class="overflow-x-auto overflow-y-auto max-h-96 flex-grow">
      <table class="w-full text-sm text-left">
        ...
        <tbody class="divide-y divide-white/5">
          <tr
            v-for="(analysis, setup) in setupAnalysis"
            :key="setup"
            class="hover:bg-white/5 transition-colors"
          >
            <td class="px-4 py-3">
              <span class="px-2 py-1 bg-purple-500/20 text-purple-300 rounded text-xs font-bold">{{
                setup
              }}</span>
            </td>
            ...
            <td
              ...
              :class="analysis.totalProfit >= 0 ? 'text-green-400' : 'text-red-400'"
            >
...
const setupAnalysis = computed(() => {
  ...
  props.positions.forEach((position) => {
    if (position.tradeInfo?.found && position.tradeInfo.trade?.setup_description) {
      ...
      // Calcular win rate
      const wins = props.positions.filter(
        (p) =>
          p.tradeInfo?.found &&
      ...
      // Calcular RR médio
      const tradesWithRR = props.positions.filter(
        (p) => p.tradeInfo?.found && p.tradeInfo.trade?.setup_description === setup
      );
