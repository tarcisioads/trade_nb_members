...
import {
  PositionHistory as PositionHistoryType,
  BingXPositionHistoryResponse,
} from '../../utils/types';
...
export class PositionHistory {
  ...
  public async getPositionHistory(params: PositionHistoryParams): Promise<PositionHistoryType[]> {
    ...
    if (useCache) {
      ...
      try {
        ...
        if (cachedData.length > 0) {
          ...
          maxCloseTimeFromCache = Math.max(
            ...cachedData.map((pos) => this.getEffectiveCloseTime(pos))
          );
          console.log(
            `Using ${cachedData.length} positions from cache, max closeTime: ${new Date(maxCloseTimeFromCache).toISOString()}`
          );
        } else {
    ...
    const queryStartTs =
      maxCloseTimeFromCache > 0 ? maxCloseTimeFromCache + 1 : (apiParams.startTs as number);
    ...
    while (currentStart <= queryEndTs) {
      ...
      console.log(
        `Making API call chunk: ${new Date(currentStart).toISOString()} to ${new Date(currentEnd).toISOString()}`
      );

      try {
        const response = await this.apiClient.get<BingXPositionHistoryResponse>(
          path,
          chunkApiParams
        );
        ...
        if (response.code !== 0 || !response.data || !response.data.positionHistory) {
          console.warn(
            `API returned non-zero code for chunk: ${response.code}, message: ${response.msg}`
          );
    ...
    console.log(
      `=== getPositionHistory END (success) - Returning ${allPositions.length} positions ===`
    );
    return allPositions;
  }
        ...
        batch.map(async (symbol) => {
          try {
            ...
            if (cachedData.length > 0) {
              ...
              const latestPosition = cachedData.sort(
                (a, b) => this.getEffectiveCloseTime(b) - this.getEffectiveCloseTime(a)
              )[0];
              startTs = this.getEffectiveCloseTime(latestPosition);
              console.log(
                `Using startTs from latest cached position for ${symbol}: ${new Date(startTs).toISOString()}`
              );
            } else {
