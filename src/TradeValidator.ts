    ...
    try {
      ...
      if ((!isValid) || (warning)) {
        ...
        if (!isSentimentValid && trade.sentiment_required) {
          message = `${message != '' ? message + ' and ' : ''}Sentiment is not good enough (${sentimentAnalysis.sentiment} - lsrTrend ${sentimentAnalysis.details.analysis.lsrTrend} - oiTrend ${sentimentAnalysis.details.analysis.oiTrend} - lsrSignal ${sentimentAnalysis.details.analysis.lsrSignal} - oiSignal ${sentimentAnalysis.details.analysis.oiSignal})`;
        }
      } else {
        let volumeStatus = '';
        if (trade.volume_required) {
        ...
        let sentimentStatus = '';
        if (trade.sentiment_required) {
          sentimentStatus = `sentiment is good (${sentimentAnalysis.sentiment} - lsrTrend ${sentimentAnalysis.details.analysis.lsrTrend} - oiTrend ${sentimentAnalysis.details.analysis.oiTrend} - lsrSignal ${sentimentAnalysis.details.analysis.lsrSignal} - oiSignal ${sentimentAnalysis.details.analysis.oiSignal})`;
        } else {
          sentimentStatus = `sentiment is optional (${sentimentAnalysis.sentiment} - lsrTrend ${sentimentAnalysis.details.analysis.lsrTrend} - oiTrend ${sentimentAnalysis.details.analysis.oiTrend} - lsrSignal ${sentimentAnalysis.details.analysis.lsrSignal} - oiSignal ${sentimentAnalysis.details.analysis.oiSignal})`;
        }
