...
export class NotificationController {
  ...
  async handleTpAdjustedMarketTrade(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Validate required fields
      if (
        !tradeNotification.symbol ||
        !tradeNotification.type ||
        !tradeNotification.entry ||
        !tradeNotification.takeProfits.tp1
      ) {
        const missingFields = [];
        if (!tradeNotification.symbol) missingFields.push('symbol');
        if (!tradeNotification.type) missingFields.push('type');
      ...
      const savedNotification =
        await this.notificationService.handleTpAdjustedMarketTrade(tradeNotification);
      res.status(200).json(savedNotification);
    } catch (error) {
