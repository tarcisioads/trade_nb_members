...
export class PositionHistoryController {
  ...
  public async getPositionHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      let positionsWithTradeInfo =
        await this.positionHistoryService.enrichPositionsWithTradeInfo(positions);

      if (onlyWithTradeInfo === 'true') {
        positionsWithTradeInfo = positionsWithTradeInfo.filter(
          (position) => position.tradeInfo?.found
        );
      }
  ...
  public async getDetailedRiskStats(req: Request, res: Response): Promise<void> {
    try {
      ...
      const detailedStats =
        this.positionHistoryService.calculateDetailedRiskStats(positionsWithTradeInfo);
      res.json({
        success: true,
        data: detailedStats,
      });
  ...
  public async getAvailableSetupDescriptions(req: Request, res: Response): Promise<void> {
    try {
      ...
      const setupDescriptions = [
        ...new Set(allTrades.map((trade) => trade.setup_description).filter(Boolean)),
      ].sort();
      res.json({
        success: true,
